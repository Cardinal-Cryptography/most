...
describe("Wrapped Token", function () {
  ...
  describe("Ownership", function () {
    it("Owner can change MinterBurner.", async function () {
      ...
      expect(await wrapped.connect(owner).setMinterBurner(owner.address)).to.not
        .be.reverted;
    ...
    it("Non-owner cannot change MinterBurner.", async function () {
      ...
      expect(wrapped.connect(nonOwner).setMinterBurner(owner.address)).to.be
        .reverted;
    });
  });
  ...
  describe("Minting", function () {
    it("MinterBurner can mint.", async function () {
      const { wrapped, minterBurner } = await loadFixture(
        setupWrappedTokenFixture,
      );
      const [_, __, nonOwner] = await hre.ethers.getSigners();
      await expect(
        wrapped.connect(minterBurner).mint(nonOwner.address, WRAP_AMOUNT),
      ).to.not.be.reverted;
  ...
  describe("Burning", function () {
    it("MinterBurner can burn.", async function () {
      ...
      await expect(wrapped.connect(minterBurner).burn(WRAP_AMOUNT)).to.be
        .reverted;

      // Allow minterBurner to take tokens from nonOwner, transfer to itself, and burn.
      await wrapped
        .connect(nonOwner)
        .approve(minterBurner.address, WRAP_AMOUNT);
      await wrapped
        .connect(minterBurner)
        .transferFrom(nonOwner.address, minterBurner.address, WRAP_AMOUNT);
      await expect(wrapped.connect(minterBurner).burn(WRAP_AMOUNT)).to.not.be
        .reverted;
