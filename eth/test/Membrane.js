...
describe("Membrane", function () {
  ...
  describe("sendRequest", function () {
    it("Reverts if the USD value of the transfer amount is below the minimum", async () => {
      const { membrane, tokenAddressBytes32 } = await loadFixture(
        deployEightGuardianMembraneFixture,
      );

      await membrane.addPair(tokenAddressBytes32, WRAPPED_TOKEN_ADDRESS);

      amountToSend = await membrane.queryPrice(
      ...
      await expect(
        membrane.sendRequest(tokenAddressBytes32, amountToSend, ALEPH_ACCOUNT),
      ).to.be.revertedWith("AmountBelowMinimum");
    });

    it("Reverts if token is not whitelisted", async () => {
      const { membrane, token, tokenAddressBytes32, membraneAddress } =
        await loadFixture(deployEightGuardianMembraneFixture);

      await token.approve(membraneAddress, TOKEN_AMOUNT);
      await expect(
        membrane.sendRequest(tokenAddressBytes32, TOKEN_AMOUNT, ALEPH_ACCOUNT),
    ...
    it("Emits correct event", async () => {
      ...
      await expect(
        ...
        .to.emit(membrane, "CrosschainTransferRequest")
  ...
  describe("receiveRequest", function () {
    ...
    it("Ignores already executed requests", async () => {
      const { membrane, token, tokenAddressBytes32 } = await loadFixture(
        deployEightGuardianMembraneFixture,
      );
      const accounts = await hre.ethers.getSigners();
      const ethAddress = addressToBytes32(accounts[10].address);
      ...
      await expect(
        ...
        .to.emit(membrane, "ProcessedRequestSigned")
  ...
  describe("payoutRewards", function () {
    it("account can request a payout", async () => {
      ...
      totalRewards = await membrane.getCollectedCommitteeRewards(
        currentCommitteeId,
        tokenAddressBytes32,
      );
    ...
    it("past committee member can still request a payout", async () => {
      ...
      await expect(await membrane.committeeId()).to.be.equal(1);
