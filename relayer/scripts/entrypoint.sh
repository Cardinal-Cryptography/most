...
if [[ -n "${RUN_TRADER}" ]]; then
  ARGS+=(
    ...
    --azero-ether-address=$(jq -r '.addresses.tokens.address[] | select(.[0] | endswith("ETH")) | .[2]' $AZERO_ADDRESSES_FILE)
    --azero-wrapped-azero-address=$(get_address $COMMON_ADDRESSES_FILE azero_wazero)
    --eth-wrapped-ether-address=$(jq -r '.addresses.tokens.address[] | select(.[0] | endswith("ETH")) | .[1]' $AZERO_ADDRESSES_FILE)
  )
fi
