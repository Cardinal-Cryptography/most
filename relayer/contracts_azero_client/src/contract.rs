...
impl ContractInstance {
    ...
    pub async fn read0<T: TryFrom<ConvertibleValue, Error = anyhow::Error>>(
        &self,
        conn: &Client,
        message: &str,
        params: ReadonlyCallParams,
    ) -> Result<T> {
        self.read::<String, T>(conn, message, &[], params).await
    }
    ...
    pub async fn exec0<S: Signer>(
        &self,
        conn: &ClientWithSigner<S>,
        message: &str,
        params: ExecCallParams,
    ) -> Result<()> {
        self.exec::<S, String>(conn, message, &[], params).await
    }

    /// Executes a contract with a given params.
    pub async fn exec<Si: Signer, S: AsRef<str> + Debug>(
        &self,
        conn: &ClientWithSigner<Si>,
        message: &str,
        args: &[S],
        params: ExecCallParams,
    ) -> Result<()> {
    ...
    pub async fn exec_dry_run<S: AsRef<str> + Debug>(
        &self,
        conn: &Client,
        sender: AccountId,
        message: &str,
    ...
    async fn dry_run_any<S: AsRef<str> + Debug>(
        &self,
        conn: &Client,
        message: &str,
        args: &[S],
